...
{
	fprintf(stderr,
			"usage: LocalContentUpdater [-r [-s]] { -v | -c | -a | -x } [--] swf-file [...]\n");
	fprintf(stderr,
			"  -r: recursive operation; specify files and/or directories\n");
	fprintf(stderr,
			"  -s: ignore all except .swf files\n");
	fprintf(stderr,
			"  -v: display version and exit\n");
	fprintf(stderr,
			"  -c: check for network privileges\n");
	fprintf(stderr,
			"  -a: add network privileges if not present\n");
	fprintf(stderr,
			"  -x: remove network privileges if present\n");
	fprintf(stderr,
			"  --: end of flags (allows filenames that start with dashes)\n");
}


// NotSwfPred: STL predicate helper that returns true if its argument does NOT
// end in ".swf".
//
struct NotSwfPred : public unary_function< string, bool >
{
	bool operator()(string path) {
		return ((path.size() < 5) ||
				(path.substr(path.size() - 4).compare(".swf") != 0));
	}
};

// getFiles: expands rPaths, which may contain file and directory paths,
//   into rFilesOut, which will contain only file paths, taken recursively
//   from all the files and directories of rPaths.
// Uses readDir and isDir helpers from readDir.h.
// Calls itself recursively for directory paths.
//
static void getFiles(const vector< string > &rPaths, vector< string > &rFilesOut)
{
	/* no extra parentheses here: */
	for (vector< string >::const_iterator i = rPaths.begin();
		 (i != rPaths.end()); i++) {
		if (isDir(*i)) {
...
int main(int argc, char *argv[])
{
	...
	while ((curArg < argc)
		...
		if (!strcmp(argv[curArg], "-v")) {
			newMode = modeVersion;
		} else if (!strcmp(argv[curArg], "-c")) {
			newMode = modeCheck;
		} else if (!strcmp(argv[curArg], "-a")) {
			newMode = modeAdd;
		} else if (!strcmp(argv[curArg], "-x")) {
			newMode = modeRemove;
		}
		...
		if (newMode != modeUnknown) {
			...
			mode = newMode;
		} else { // Non-exclusive flags
			if (!strcmp(argv[curArg], "-r")) {
				modeRecursive = true;
	...
	// If -s was specified, delete all paths that do NOT end in .swf
	if ( modeSwf ) {
		// If this code looks weird, thank STL.
		// See http://www.sgi.com/tech/stl/remove_if.html
		pFiles->erase(remove_if(pFiles->begin(), pFiles->end(), NotSwfPred()),
					  pFiles->end());
	}

	// Display number of paths
	printf("Operating on %d %s.\n", (int)pFiles->size(),
		   (pFiles->size() == 1 ? "file" : "files"));
	...
	/* no extra parentheses here: */
	for (vector< string >::const_iterator i = pFiles->begin();
		 (i != pFiles->end()); i++) {
		string swfPath = *i;
		...
		// If the task caused the contents of the file to change, then
